(async ()=>{
  ...
  const alt = `function solve(input){ const nums = (String(input||'').match(/-?\\d+/g) || []).map(Number); return String(nums.reduce((a,b)=>a+b,0)); }`
  console.log('Running alt solver');
  const res = await post(`/api/tasks/${taskId}/run-demo`, token, { code: alt, language: 'javascript' })
